...
The [`generate_docs()`](generate_docs.py) script generates a number of markdown pages in the docs, while also updating other markdown pages with concept descriptions from the [_source](_source) directory. These same description files are used by the [OpenAD Toolkit] to generate the help output, ensuring consistency across all documentation.

### How to regenerate documentation

    python3 generator/generate_docs.py

This will export all files into the [_output](_output) folder. You will then be promted if you want to copy the files to their final destination, which will overwrite the current files.
